...
const Avatars = () => {
    ...
    return (<>
      ...
      <ScrollView>
        {_.chunk(dataList, 3).map((chunk, chunkIndex) => (<View style={{
                flexDirection: 'row',
                justifyContent: 'space-around',
                marginTop: 10,
                backgroundColor: 'grey',
            }} key={chunkIndex}>
            {chunk.map((l, i) => (<Avatar size={64} rounded source={l.image_url ? { uri: l.image_url } : null} icon={l.icon} title={l.title} key={`${chunkIndex}-${i}`}/>))}
          </View>))}
        <View style={{
            flexDirection: 'row',
            justifyContent: 'space-around',
          ...
          <Avatar size={64} rounded source={{
            uri: 'https://randomuser.me/api/portraits/women/63.jpg',
        }}>
            <Avatar.Accessory size={24}/>
          </Avatar>
          <Avatar size={64} rounded icon={{ name: 'user', type: 'font-awesome' }}>
        ...
        <View style={{
          ...
          <ThemeProvider theme={theme}>
            ...
            <Button title="My 2nd Button" onPress={() => {
            if (theme === theme1) {
                setTheme(theme2);
            }
            else {
